...
// This is the bot original code with everything unified in one file.
...
class TradingBot {
    ...
    async performSwap(token, isBuy) {
        logger.info(
            ...
            } [${this.keypair.publicKey.toBase58()}] [${token.token.symbol}] [${token.token.mint
            }]`
        );
    ...
    logTransaction(txid, isBuy, token) {
        logger.info(
            `${isBuy ? chalk.green("[BOUGHT]") : chalk.red("[SOLD]")} ${token.token.symbol
            } [${txid}]`
        );
    }
