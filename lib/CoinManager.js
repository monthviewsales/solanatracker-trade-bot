        ...
        if (tokenEntry.balance > 0) {
          ...
          const coin = {
            token: {
              ...
              // Optionally add a symbol if available, e.g. symbol: tokenEntry.symbol
            },
            status: coinStatus,
            position: {
  ...
  async fillMissingPositionData(coin, publicKey) {
    if (!coin.position || !Number.isFinite(coin.position.entryPrice)) {
      try {
        ...
        logger.error(`[CoinManager] Error fetching trade history for ${tokenSymbol}: ${err.message}`, { error: err });
      }
    }
  }
}
