...
Final Buy Decision: ${decision}`);
    ...
    evaluateSell(entry, position, config) {
        ...
        let standardSell = signalScore >= minSellScore;

        logger.info(`[Indicators] Calculated Sell Signal Score: ${signalScore} (Minimum Required: ${minSellScore}) | PnL: ${pnl.toFixed(2)}%`);
