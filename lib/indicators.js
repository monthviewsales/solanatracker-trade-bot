    ...
    */
    evaluateSell(entry, position, config) {
        ...
        if (priceBelowStopLoss) {
            ...
            logger.info(`[Indicators] Sell Trigger: Trailing Stop activated. Current Price: ${ind.price} is below threshold (${(position.highestPrice * (1 - adaptiveTrailingStopPercent)).toFixed(2)}) based on Highest Price: ${position.highestPrice}.`);
            shouldSell = true;
        } else if (trailingTakeProfitTriggered) {
            logger.info(`[Indicators] Sell Trigger: Trailing Take Profit activated. Current Price: ${ind.price} is above profit threshold (${(position.highestPrice * (1 + trailingStopPercentEnv)).toFixed(2)}) based on Highest Price: ${position.highestPrice}.`);
            shouldSell = true;
...
Trailing Stop Threshold: ${position.highestPrice ? (position.highestPrice * (1 - adaptiveTrailingStopPercent)).toFixed(2) : 'N/A'} (Triggered: ${trailingStopTriggered})
Trailing Take Profit Threshold: ${position.highestPrice ? (position.highestPrice * (1 + trailingStopPercentEnv)).toFixed(2) : 'N/A'} (Triggered: ${trailingTakeProfitTriggered})
