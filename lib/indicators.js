...
module.exports = {
    ...
    evaluateTrade(entry, config) {
        ...
        let decision;
        if (logicMode === "strict") {
            decision = ind.trendBias && ind.price <= bbTarget && ind.rsi <= rsiThreshold;
        } else {
            decision = ind.trendBias && (ind.price <= bbTarget || ind.rsi <= rsiTarget);
        }
...
Trend Bias: ${ind.trendBias}
Buy Logic Mode: ${logicMode}

Buy Conditions:
  Strict Condition -> Price <= BB Target: ${ind.price <= bbTarget} AND RSI <= RSI Threshold: ${ind.rsi <= rsiThreshold}
