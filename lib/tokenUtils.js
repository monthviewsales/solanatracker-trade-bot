...
function filterTokens(rawTokens, coinStore) {
  ...
  const filtered = rawTokens.filter((entry) => {
    ...
    if (existing && ["open", "closed", "blacklist", "target"].includes(existing.status)) {
      logger.warn(`⚠️ [TokenUtils] Token ${token.symbol} skipped due to existing status: ${existing.status}`);
      return false;
    }
  ...
  filtered.forEach(token => {
      logger.info(`[TokenUtils] Passed token: ${token.token.symbol} (${token.token.mint})`);
  });
