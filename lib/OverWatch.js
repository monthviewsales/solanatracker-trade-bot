...
class Overwatch {
  ...
  async loadPositions() {
    try {
      ...
      logger.info(`Loaded ${this.positions.size} positions from ${this.positionsFile}`);
    } catch (err) {
      if (err.code === "ENOENT") {
  ...
  async savePositions() {
    try {
      ...
      logger.info(`Saved ${this.positions.size} positions to ${this.positionsFile}`);
    } catch (err) {
  ...
  async loadSoldPositions() {
    try {
      ...
      logger.info(`Loaded ${this.soldPositions.length} sold positions from file`);
    } catch (err) {
      if (err.code !== "ENOENT") {
  ...
  async validatePositions() {
    logger.info("Validating wallet balance for active positions...");
    let removed = 0;
    ...
    for (const [mint, position] of this.positions.entries()) {
      ...
      if (!balance || balance === 0) {
        logger.warn(`Removing stale position ${position.symbol} (${mint}) - no balance.`);
    ...
    if (removed > 0) {
      await this.savePositions();
      logger.info(`Removed ${removed} stale position(s).`);
    } else {
  ...
  tagBuy({ mint, qty, entryPrice, txid }) {
    ...
    logger.info(`Tagged BUY for ${coin.token.symbol} (${mint})`);
  }
  ...
  tagSell({ mint, qty, exitPrice, txid }) {
    ...
    this.coinStore.addOrUpdate(coin);
    logger.info(`Tagged SELL for ${coin.token.symbol} (${mint}) — PnL: ${pnl.toFixed(6)} (${pnlPct.toFixed(2)}%)`);
  }
}
