...
class Overwatch {
    ...
    try {
      ...
      try {
        ...
        for (const [mint, position] of this.positions.entries()) {
          if (!position.highestPrice || position.highestPrice === undefined || position.highestPrice === null) {
              position.highestPrice = position.entryPrice;
          }
          if (!position.sl || position.sl === undefined || position.sl === null) {
              position.sl = position.entryPrice * (1 - trailingStopPercent);
          }
          logger.info(`[Overwatch] Ensured highestPrice and sl for ${position.symbol} during load.`);
        }
  ...
  async validatePositions() {
    ...
    for (const [mint, position] of this.positions.entries()) {
      let balance;
      try {
        if (!this.walletManager || typeof this.walletManager.getWalletAmount !== "function") {
            logger.error(`[Overwatch] Wallet manager not initialized correctly. Cannot fetch wallet amount.`);
            continue;
        }
    ...
    if (removed > 0 || updated) {
      ...
      if (removed > 0) {
          ...
          logger.info("✅ [Overwatch] Updated positions validated successfully");
      }
    } else {
  ...
  async tagBuy({ mint, qty, entryPrice, txid }) {
    try {
      if (entryPrice === 0) {
        logger.warn(`tagBuy: Entry price is zero for mint ${mint}. Position data is malformed.`);
      }
      const coin = this.coinStore.findByMint(mint);
      if (!coin.token || !coin.token.mint) {
        logger.error(`tagBuy: Coin for mint ${mint} does not have a valid token.mint property.`);
        return;
      }
      ...
      const amount = qty || 1; // Default to 1 if qty is not specified

      coin.position = { qty: amount, entryPrice, highestPrice: entryPrice, sl: stopLoss, txid, timestamp };
      ...
      this.coinStore.addOrUpdate(coin);
      logger.info(`🟢 [BUY] ${coin.token.symbol} (${mint}) — qty: ${amount}, price: ${entryPrice}`);
    } catch (err) {
  ...
  async tagSell({ mint, qty, exitPrice, txid }) {
    try {
      ...
      if (!coin.token || !coin.token.mint) {
        logger.error(`tagSell: Coin for mint ${mint} does not have a valid token.mint property.`);
        return;
      }
      ...
      if (entryPrice === 0) {
        logger.warn(`tagSell: Entry price is zero for mint ${mint}. Position data is malformed, but proceeding with sell operation.`);
      }
